...
-- Options:
local camera = game.FindFirstChildOfClass(workspace, "Camera");
...
local function connectLoop()
	...
	table.insert(connections, RunService.RenderStepped:connect(function()
		...
		local part, position = raycast(Ray.new(camera.CFrame.p, -rainDirection * RAIN_SCANHEIGHT), true)
		
		if (not currentCeiling or camera.CFrame.p.y <= currentCeiling) and not part then
			...
			Emitter.CFrame =
				...
				+ (1 - t) * camera.CFrame.lookVector * Emitter.Size.Z/3
		...
		if frame >= RAIN_UPDATE_PERIOD then
			...
			if inside then
				...
				if not disabled then
					...
					if (not currentCeiling or camera.CFrame.p.y <= currentCeiling) then
						...
						for i = 1, #volumeScanGrid do -- In order, so first hit is closest
							if not raycast(Ray.new(camera.CFrame * volumeScanGrid[i], rayDirection), true) then
